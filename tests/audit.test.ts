...
import { createRecoveryAuditRecorder, writeAuditEntry } from "../src/audit.js";
...
describe("RecoveryAuditEntry", () => {
  it("persists one final entry when duplicate terminal signals arrive", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "audit-recovery-"));
    try {
      const auditPath = path.join(dir, "audit.jsonl");
      const recorder = createRecoveryAuditRecorder({ auditPath });
      const entry = {
        kind: "recovery" as const,
        timestamp: "2026-07-13T00:00:00.000Z",
        callID: "call-1",
        event: "fallback_succeeded" as const,
      ...
      const lines = (await readFile(auditPath, "utf8")).trim().split("\n");
      expect(lines).toHaveLength(1);
      expect(JSON.parse(lines[0]!)).toMatchObject({ kind: "recovery", event: "fallback_succeeded" });
    } finally {
      await rm(dir, { recursive: true, force: true });
    }
  });
